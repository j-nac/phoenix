...
function LogoSelect(props) {
    return <a href={props.link} className="text-xl my-auto text-white font-paragraph flex items-center"><img src={icon2} alt="" className="h-10"/>STRAL</a>
}
...
function MenuSelectPage(props) {
    return <a className='text-white before:z-[-1] before:absolute before:h-0 before:w-3/4 px-2 after:left-0 before:left-0 before:bottom-0 before:bg-pink hover:before:h-full before:transition-all after:z-[-1] py-2 after:block after:h-1 after:w-3/4 after:bg-white after:bottom-0 hover:after:bottom-full hover:after:translate-y-full after:transition-all after:absolute relative transition-color text-3xl' href={props.link}>{props.label}</a>
}
